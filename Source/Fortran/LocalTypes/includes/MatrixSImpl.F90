...
PURE FUNCTION ConstructEmptyMatrix(rows, columns, zero_in) RESULT(this)
  !! Parameters
  TYPE(SMTYPE) :: this
...
FUNCTION ConstructMatrixFromFile(file_name) RESULT(this)
  !! Parameters
  TYPE(SMTYPE) :: this
  ...
  CALL SortTripletList(triplet_list, temp_columns, temp_rows, &
       & sorted_triplet_list)
  this = ConstructMatrixFromTripletList(sorted_triplet_list, temp_rows, &
       & temp_columns)

  CALL DestructTripletList(triplet_list)
  CALL DestructTripletList(sorted_triplet_list)
END FUNCTION ConstructMatrixFromFile
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!! @param[in] rows number of matrix rows
!! @param[in] columns number of matrix columns
PURE FUNCTION ConstructMatrixFromTripletList(triplet_list,rows,columns) &
     & RESULT(this)
  !! Parameters
  TYPE(SMTYPE) :: this
...
END FUNCTION ConstructMatrixFromTripletList
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
PURE SUBROUTINE ExtractMatrixRow(this, row_number, row_out)
  ...
  !! Fill a value buffer
  row_out = ConstructEmptyMatrix(1,this%columns)
...
PURE SUBROUTINE ExtractMatrixColumn(this, column_number, column_out)
  ...
  !! Allocate Memory
  column_out = ConstructEmptyMatrix(this%rows, 1)
...
PURE SUBROUTINE TransposeMatrix(this, matT)
  ...
  matT = ConstructEmptyMatrix(this%columns, this%rows)
...
PURE SUBROUTINE ComposeMatrixColumns(mat_list, out_matrix)
  ...
  !! Allocate The Space
  out_matrix = ConstructEmptyMatrix(mat_list(1)%rows, total_columns)
...
PURE SUBROUTINE SplitMatrixColumns(this, num_blocks, block_sizes, &
  ...
  DO split_counter = 1, num_blocks
     ...
     !! Construct
     split_list(split_counter) = ConstructEmptyMatrix(this%rows, lcolumns)
     !! Copy Outer Index
...
PURE SUBROUTINE MatrixToTripletList(this, triplet_list)
  ...
  triplet_list = TLISTTYPE(size_of_this)
