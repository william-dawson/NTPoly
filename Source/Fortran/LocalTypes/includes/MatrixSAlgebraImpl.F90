...
PURE SUBROUTINE ScaleMatrix(matA,constant)
  !! Parameters
...
PURE SUBROUTINE IncrementMatrix(matA, matB, alpha_in, threshold_in)
  !! Parameters
  ...
  matC = SMTYPE(matA%rows, matA%columns)
  ...
  matB = SMTYPE(matC%rows, matC%columns)
...
PURE SUBROUTINE PairwiseMultiplyMatrix(matA, matB, matC)
  !! Parameters
  TYPE(SMTYPE), INTENT(IN)  :: matA
  TYPE(SMTYPE), INTENT(IN) :: matB
  TYPE(SMTYPE), INTENT(INOUT) :: matC
  !! Counter Variables
  INTEGER :: outer_counter
  INTEGER :: elements_per_inner_a, elements_per_inner_b
  INTEGER :: total_counter_a, total_counter_b, total_counter_c
  ...
  TempMat = SMTYPE(matA%rows, matA%columns)
  size_of_a = matA%outer_index(matA%columns+1)
  size_of_b = matB%outer_index(matB%columns+1)
  ...
  !! Cleanup
  CALL DestructMatrix(matC)
  matC = SMTYPE(TempMat%rows, TempMat%columns)
...
END SUBROUTINE PairwiseMultiplyMatrix
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!> Product = sum(MatA[ij]*MatB[ij])
!! @param[in] matA Matrix A.
!! @param[in] matB Matrix B.
!! @result product
PURE FUNCTION DotMatrix(matA, matB) RESULT(product)
  !! Parameters
  TYPE(SMTYPE), INTENT(IN) :: matA
  TYPE(SMTYPE), INTENT(IN) :: matB
  DATATYPE :: product
...
!! calculation.
SUBROUTINE GemmMatrix(matA, matB, matC, IsATransposed_in, IsBTransposed_in, &
  ...
  REAL(NTREAL) :: beta
  REAL(NTREAL) :: threshold
  TYPE(MPOOLTYPE) :: blocked_memory_pool
  ...
  ELSE
     ...
     IF (.NOT. PRESENT(blocked_memory_pool_in)) THEN
        blocked_memory_pool = MPOOLTYPE(mat_c_columns, mat_c_rows, &
             & sparsity_estimate)
        pool_flag = .FALSE.
     ELSEIF (.NOT. CheckMemoryPoolValidity(blocked_memory_pool_in, &
        ...
        blocked_memory_pool_in = MPOOLTYPE(mat_c_columns, mat_c_rows, &
             & sparsity_estimate)
        pool_flag = .TRUE.
     ELSE
...
PURE SUBROUTINE MatrixColumnNorm(this, norm_per_column)
  !! Parameters
  ...
  DO outer_counter = 1, this%columns
     ...
     END DO
  END DO
END SUBROUTINE MatrixColumnNorm
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!> Compute the 1 norm of a sparse matrix.
!! @param[in] this the matrix to compute the norm of.
!! @result norm the matrix.
PURE FUNCTION MatrixNorm(this) RESULT(norm)
  !! Parameters
  TYPE(SMTYPE), INTENT(IN) :: this
  REAL(NTREAL) :: norm
  !! Local Variables
...
PURE FUNCTION MatrixGrandSum(this) RESULT(sum_value)
  !! Parameters
  TYPE(SMTYPE), INTENT(IN) :: this
  DATATYPE :: sum_value
...
PURE SUBROUTINE SparseBranch(matA, matB, matC, IsATransposed, IsBTransposed, &
  ...
  TYPE(MPOOLTYPE), INTENT(INOUT) :: blocked_memory_pool
  !! Local Data
  TYPE(SMTYPE) :: matAT, matBT
...
SUBROUTINE DenseBranch(matA, matB, matC, IsATransposed, IsBTransposed, &
  ...
  REAL(NTREAL), INTENT(IN) :: threshold
  !! Local Data
  TYPE(SMTYPE) :: untransposedMatA
  TYPE(SMTYPE) :: untransposedMatB
  TYPE(DMTYPE) :: DenseA
  TYPE(DMTYPE) :: DenseB
  TYPE(DMTYPE) :: DenseC
...
PURE SUBROUTINE MultiplyBlock(matAT,matBT,memorypool)
  !! Parameters
  TYPE(SMTYPE), INTENT(IN)  :: matAT
  TYPE(SMTYPE), INTENT(IN)  :: matBT
  TYPE(MPOOLTYPE), INTENT(INOUT) :: memorypool
  !! Temp Variables
  DATATYPE :: temp_value_a, temp_value_b, temp_value_c
...
PURE SUBROUTINE PruneList(memorypool,alpha,threshold, &
  ...
  TYPE(SMTYPE), INTENT(INOUT) :: matAB
  !! Local data
  INTEGER :: row_counter_c, column_counter_c, hash_counter
  DATATYPE :: working_value
  INTEGER :: working_column
  INTEGER :: temp_values_per_hash
  TYPE(TLISTTYPE) :: unsorted_pruned_list
  TYPE(TLISTTYPE) :: sorted_pruned_list
  ...
  DO row_counter_c = 1, mat_c_rows
     ...
     END DO
  END DO
  unsorted_pruned_list = TLISTTYPE(pruned_counter-1)
  ...
  matAB = SMTYPE(sorted_pruned_list,mat_c_rows, mat_c_columns)
