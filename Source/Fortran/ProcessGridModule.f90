...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE DestructProcessGrid(grid)
    ...
    ! CALL MPI_COMM_FREE(grid%row_comm, ierr)
    ! CALL MPI_COMM_FREE(grid%column_comm, ierr)
    ! CALL MPI_COMM_FREE(grid%within_slice_comm, ierr)
    ! CALL MPI_COMM_FREE(grid%between_slice_comm, ierr)
    ! CALL MPI_COMM_FREE(grid%global_comm, ierr)

    !! Deallocate Blocks
    IF (ALLOCATED(grid%blocked_row_comm)) THEN
       ! DO II=1,grid%number_of_blocks_rows
       !    CALL MPI_COMM_FREE(grid%blocked_row_comm(II), ierr)
       ! END DO
       DEALLOCATE(grid%blocked_row_comm)
    END IF
    IF (ALLOCATED(grid%blocked_column_comm)) THEN
       ! DO JJ=1,grid%number_of_blocks_columns
       !    CALL MPI_COMM_FREE(grid%blocked_column_comm(JJ), ierr)
       ! END DO
    ...
    IF (ALLOCATED(grid%blocked_within_slice_comm)) THEN
       ! DO JJ=1,grid%number_of_blocks_columns
       !    DO II=1,grid%number_of_blocks_rows
       !       CALL MPI_COMM_FREE(grid%blocked_within_slice_comm(II,JJ), ierr)
       !    END DO
       ! END DO
    ...
    IF (ALLOCATED(grid%blocked_between_slice_comm)) THEN
       ! DO JJ=1,grid%number_of_blocks_columns
       !    DO II=1,grid%number_of_blocks_rows
       !       CALL MPI_COMM_FREE(grid%blocked_between_slice_comm(II, JJ), ierr)
       !    END DO
       ! END DO
