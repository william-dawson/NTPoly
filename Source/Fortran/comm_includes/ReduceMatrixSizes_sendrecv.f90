  ...
  ALLOCATE(helper%size_send_request_list(helper%comm_size))
  ALLOCATE(helper%size_recv_request_list(helper%comm_size))
  DO II = 1, helper%comm_size
     CALL MPI_Isend(SIZE(matrix%values), 1, MPI_INT, II-1, 1, communicator, &
          & helper%size_send_request_list(II), grid_error)
     CALL MPI_Irecv(helper%values_per_process(II:II), 1, MPI_INT, II-1, 1, &
          & communicator, helper%size_recv_request_list(II), &
          & grid_error)
  END DO
